...
public class IndexController extends BaseController {
    ...
    public String index(HttpServletRequest request, @PathVariable int pageNumber, @RequestParam(value = "limit", defaultValue = "12") int limit) {
        pageNumber = pageNumber < 0 || pageNumber > WebConst.MAX_PAGE ? 1 : pageNumber;
        PageInfo<ContentVo> articles = contentService.getContents(pageNumber, limit);
        request.setAttribute("articles", articles);
        if (pageNumber > 1) {
            this.title(request, "第" + pageNumber + "页");
        }
