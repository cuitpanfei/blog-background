...
public class DateKit {
    ...
    public static boolean isToday(Date date) {
        ...
        LocalDate from = LocalDateTime.ofInstant(date.toInstant(), ZoneId.systemDefault())
                .toLocalDate();
        boolean result = now.isEqual(from);
        return result;
    }
    ...
    public static Date getDaysAgoDate(int interval) {
        return getDaysWithInterval(-interval);
    }

    public static Date getDaysAfterDate(int interval) {
        return getDaysWithInterval(interval);
    }

    public static Date getDaysWithInterval(int interval) {
        LocalDate now = LocalDate.now();
        ZonedDateTime zonedDateTime;
        if (interval == 0) {
            zonedDateTime = now.atStartOfDay(ZoneId.systemDefault());
        } else {
            zonedDateTime = LocalDate.ofEpochDay(now.toEpochDay() + interval)
    ...
    public static String getDaysAgo(int interval) {
        Date date = getDaysAgoDate(interval);
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        try {
            return format.format(date);
    ...
    public static Date getStartDay(int year, int month) {
        ZonedDateTime zonedDateTime = LocalDate.of(year, month, 1)
                .atStartOfDay(ZoneId.systemDefault());
        return Date.from(zonedDateTime.toInstant());
    }
    ...
    public static Date dateAdd(int interval, Date date, int n) {
        class Type {
            public final static int DAY = 1, WEEK = 2, MONTH = 3, YEAR = 4, HOUR = 5, MINUTE = 6, SECOND = 7;
        }
